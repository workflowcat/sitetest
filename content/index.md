---
title: Ласкаво просимо
description: Персональна база знань та цифровий сад
---

![Цифровий сад](https://images.unsplash.com/photo-1416879595882-3373a0480b5b?w=1200&q=80)

Це мій цифровий сад — місце для думок, ідей та нотаток, що ростуть і розвиваються з часом.

> [!info] Що таке цифровий сад?
> Цифровий сад — це колекція взаємопов'язаних нотаток та ідей. На відміну від блогу, тут немає хронологічного порядку. Нотатки еволюціонують, доповнюються та з'єднуються між собою.

## Навігація

- 📝 [[notes/index|Нотатки]] — Колекція думок, ідей та знань на різні теми
- 🚀 [[projects|Проєкти]] — Поточні та завершені проєкти з документацією
- ℹ️ [[about|Про сайт]] — Як користуватися цим сайтом та що тут можна знайти

## Особливості

- **Markdown** — весь контент написаний простим текстом
- **Темна тема** — за замовчуванням, з можливістю перемкнути на світлу
- **Пошук** — швидкий пошук нотаток (⌘K)
- **Граф зв'язків** — візуалізація зв'язків між нотатками
- **Зворотні посилання** — бачте, які сторінки посилаються на поточну

## Останні нотатки

| Назва | Тема | Статус |
|-------|------|--------|
| [[notes/pkm-intro|Вступ до PKM]] | Продуктивність | 🌱 |
| [[notes/markdown-tips|Поради з Markdown]] | Написання | 🌳 |
| [[notes/building-in-public|Публічне навчання]] | Мета | 🌿 |
